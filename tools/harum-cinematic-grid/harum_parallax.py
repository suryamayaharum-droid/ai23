...
def main():
    ...
    h,w=img.shape[:2]
    dep=cv2.resize(dep,(w,h),interpolation=cv2.INTER_CUBIC).astype(np.float32)/255.0
    ...
    for i in range(n):
        ...
        writer.write(frame)
    writer.release()
    subprocess.run(["ffmpeg","-y","-i",str(tmp),"-c:v","libx264","-crf","18","-preset","medium",
                    "-pix_fmt","yuv420p","-movflags","+faststart","-an",a.output],check=True)
