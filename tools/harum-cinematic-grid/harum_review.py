...
def main():
    ...
    ap.add_argument("--out",default="review.jpg"); ap.add_argument("--frames",type=int,default=9)
    a=ap.parse_args()
    ...
    d=float(subprocess.check_output(["ffprobe","-v","error","-show_entries","format=duration",
                                     "-of","default=nw=1:nk=1",a.video],text=True).strip())
    fps=a.frames/max(d,0.1); cols=3; rows=(a.frames+cols-1)//cols
    subprocess.run(["ffmpeg","-y","-i",a.video,"-vf",
                    f"fps={fps:.8f},scale=320:-1,tile={cols}x{rows}:padding=6:margin=6",
