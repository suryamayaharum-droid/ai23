...
def add(statement,status='hypothesis',source_type='llm',source_ref=None):
    if status not in {'hypothesis','verified','rejected'}:
        raise ValueError('bad status')
    i=hashlib.sha256((statement+source_type+str(source_ref)).encode()).hexdigest()[:20]
    now=time.time()
    c=conn()
    c.execute('INSERT OR REPLACE INTO claims(id,statement,status,source_type,source_ref,created,updated) VALUES(?,?,?,?,?,?,?)',
              (i,statement,status,source_type,source_ref,now,now))
    c.commit();c.close()
    return i

def promotable(ids):
    c=conn()
    rows=[c.execute('SELECT status FROM claims WHERE id=?',(i,)).fetchone() for i in ids]
    c.close()
...
if __name__=='__main__':
    ...
    a.add_argument('--source-type',default='llm')
    a.add_argument('--source-ref')
    p=sp.add_parser('promotable')
