...
def xfade(inputs,out,transition=0.16,effect="fade",fps=30,crf=18,preset="veryfast"):
    ...
    current="v0"; elapsed=durs[0]
    for i in range(1,len(inputs)):
        offset=max(0.0,elapsed-transition); outlabel=f"x{i}"
        filters.append(f"[{current}][v{i}]xfade=transition={effect}:duration={transition}:offset={offset:.6f}[{outlabel}]")
        current=outlabel; elapsed+=durs[i]-transition
    cmd+=["-filter_complex",";".join(filters),"-map",f"[{current}]",
          "-c:v","libx264","-crf",str(crf),"-preset",preset,
          "-pix_fmt","yuv420p","-movflags","+faststart","-an",str(out)]
    sh(cmd,quiet=True); return elapsed

def ambience(video,out,dur,level=0.018):
    filt=(f"anoisesrc=color=pink:amplitude={level}:duration={dur}:sample_rate=48000,"
...
def main():
    ...
    if a.no_ambience: visual.replace(final)
    else:
        ambience(visual,final,dur); visual.unlink(missing_ok=True)
    print(json.dumps({"output":str(final),"duration":dur,
      "shots":len(inputs),"transition":a.transition,"effect":a.effect},indent=2))
